<?php
/**
* Adds new shortcode "info-Vacacional-listado" and registers it to
...
if ( ! class_exists( 'vcVacacionalSlider' ) ) {

	class vcVacacionalSlider {
		public function __construct() {
	            add_shortcode( 'info-vacacional-listado', array( 'vcVacacionalSlider', 'output' ) );
			if ( function_exists( 'vc_lean_map' ) ) {
	            vc_lean_map( 'info-vacacional-listado', array( 'vcVacacionalSlider', 'map' ) );
			}
		}
		...
		public static function map() {
			...
			return array(
                'name'        => esc_html__( 'Vacacional', 'Jaluvava-living' ),
                'description' => esc_html__( 'Vacacional', 'Jaluvava-living' ),
				'base'        => 'vc_infobox',
				...
				'admin_enqueue_js' => array(
					plugins_url().'/componentes/js/vc-Vacacional.js?t='.time()
				),
				/*'front_enqueue_js' => array(
					plugins_url().'/componentes/js/vc-testimonios-front.js?t='.time()
				),*/
				'custom_markup' => '<h2 class="wpb_element_title"> <i class="vc_general vc_element-icon"></i>Vacacional</h4><span class="vc_admin_label admin_label_content vc_custom-element-container" style="display: inline;"><label>Vacacional</label>: {{{ params.tipo_contenido }}} - {{{ params.inv_categoria }}} </h2>',
		...
		*/
		public static function output( $atts, $content = null ) {
			...
			$values = vc_param_group_parse_atts($atts['vacacional']);
			$html = '';

			$html .= '<div class="vacacional-slider columnas   ">';
				$html .= '<div class="slick vacacional-slick-slide ">';
			...
			$html .= '<script>
				+(function ($) {
				  $(function(){
				  	...
				  	console.log(urlHash,$(\'.vacacional-sliderr\'), $(\'.vacacional-slider .vacacional-det\').index( $(urlHash) ) );
				  	var indexToGet = $(\'.vacacional-slider .vacacional-det\').index( $(urlHash) );
